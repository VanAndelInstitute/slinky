...
# TODO: Handle more than 1000 ids
...
Slinky$methods(clue = function(endpoint=c("sigs", "cells", "genes", "perts", "plates", "profiles", "rep_drugs", "pcls"), 
  ...
  \\item{\\code{ids} Optional vector of ids to fetch for sigs or profiles endpoints.  Should not be used together with where_clause or count.}
  ...
  if(sum(length(ids) & count & length(where_clause)) > 1) {
    stop("In call to Slinky$clue, the ids, where_clause, and count arguments are mutually exclusive.  Ensure only 1 provided.")
  }
  
  if(length(ids) && !endpoint %in% c("sigs", "profiles")  ) {
    stop("Specifying id list currently only supported for sigs and profiles endpoints")
  }
  ...
  # we cannot retrieve more than 1000 records at a time,
  # and trying to send more than 1000 ids in an inq statement also does not work.
  # so throttle to 1000 records per request, unless user has requested fewer
  if(limit < 1000) {
    lim_filter <- limit
  } else {
    lim_filter <- 1000
  ...
  dat <- foreach::foreach(i=1:ceiling(count/1000), 
                          ...
                          .packages = c("httr", "jsonlite")) %dopar% 
                          {
                            ...
                            ii <- (i-1)*1000
                            if(length(ids)) {
                              where_clause = list("distil_id"= list("inq"=c(ids[ii:(ii+lim_filter)])))
                              skip <- 0;
                            } else {
                              skip <- ii
                            }
                            ...
                            dplyr::bind_rows(lapply(fl, as.data.frame, stringsAsFactors=FALSE))
                          }
  if(length(dat) == 0) {
    return(NULL)
  }
...
Slinky$methods(clue.vehicle = function(ids, verbose=FALSE) {
  ...
  \\subsection{Details}{This is a convenience wrapper to the profiles API which queries clue.io and unwraps response. }"
  if(length(ids) > 1) {
    where_clause = list(distil_id = list("inq"=c(ids)))
  } else {
    where_clause = list(distil_id = ids)
  }
  res <- .self$clue("profiles", fields=c("pert_id", "pert_iname", "pert_vehicle"), where_clause = where_clause, verbose = verbose)
  return(res)
})
...
#' @export
Slinky$methods(clue.instances = function(where_clause = NULL, verbose=FALSE, poscon=c("keep", "omit")) {
  "Convenience wrapper to query function to retrieve instance ids meeting specified criteria.
  \\subsection{Parameters}{
  \\itemize{
  \\item{\\code{where_clause} Filter terms, as a list of terms, e.g. \\code{list(pert_type='trt_cp', 'is_gold'=TRUE)}.  Terms will be joined by AND logic.}
  \\item{\\code{verbose} Do you want to know how things are going? Default is false.}
  \\item{\\code{poscon} Instances of type \\code{trt_poscon} are recoded as \\code{trt_cp} in clue.io's \\code{sigs} endpoint.  
                        This can lead to unexpected results downstream.  To drop these instances, specify \\code{poscon='omit'}}
  }}
  ...
  ids <- .self$clue("sigs", fields = c("distil_id"), where_clause = where_clause, unpack_sigs = TRUE, verbose = verbose)
  ids <- as.character(unlist(sapply(ids[,1], function(x) { strsplit(x, "\\|") })))

  if(poscon == "omit") {
    pc <- readRDS(system.file("extdata", "trt_poscon.rds", package="slinky"))
