...
Slinky$methods(toEset = function(gctx=NULL, 
  ...
  \\subsection{Details}{You must specify exactly one of index, ids or where_clause to avoid inadvertently slurping in entire gctx file.}"
  
  if(sum(!length(index), !length(ids), !length(where_clause)) != 2) {
    stop("toEset function requires exactly one of index, ids, or where_clause.")
  }
  ...
  if(length(index)) {
    ...
    if(!length(ix)) {
      warning("Specified where_clause or ids returned no results.  Please verify your query/ids.")
      return(NULL)
    } 
