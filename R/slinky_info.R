#' @export
Slinky$methods(loadInfo = function(file = NULL, verbose = FALSE,
    reload = FALSE) {
    ...
    tryCatch({
        if (verbose)
            message("\nAttempting to load info file.  This may take a couple ",
                "minutes (but only needs to be done once per session).")
...
Slinky$methods(.locateInfo = function(verbose = FALSE) {
    "Internal function to locate LINCS instance info datafile and download if
        missing.
    \\subsection{Parameters}{
    \\itemize{
        \\item{\\code{path} Optional path of where to look.  Default is to look
            in installation directory and current working directory.}
        \\item{\\code{verbose} Do you want to know how things are going?
            Default is FALSE}}}
    \\subsection{Return Value}{Path to info file (downloaded if missing as a
            side effect)}
    ...
    if (!info_lib && !info_wd && !info_user) {
        ...
        tryCatch({
            ...
            message("Could not move file into lib directory.  Will use from ",
...
Slinky$methods(controls = function(ids, verbose = FALSE, cl = NULL) {
    ...
    profs <- .self$clue("profiles", fields = c("pert_id", "pert_iname",
        "pert_vehicle", "pert_type", "rna_plate"), verbose = verbose,
        cl = cl, ids = ids)
    cond <- profs %>% group_by(rna_plate, pert_type, pert_vehicle) %>%
