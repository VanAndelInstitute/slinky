...
Slinky$methods(diffexp = function(treat,
    ...
    \\itemize{
        \\item{\\code{treat} A SummarizedExperiment containing the treated 
            samples, or the pert_desc of desired perturbagen. See details. }
        \\item{\\code{conrol} A SummarizedExperiment containing the control 
            samples, or the pert_desc of desired controls. Default is 'auto'. 
            See details.}
    ...
    if (class(treat) == "character") {
        ...
        treat <- .self$toSummarizedExperiment(where_clause = where_clause,
                                fields = fields,
                                verbose = verbose)
        if (verbose)
        ...
        stop("de.by.plate expects either the pert_desc of the perturbagen ",
            "or a SummarizedExperiment for the 'treat' dataset")
    }

    if (class(control) == "character") {
        if(control == "auto") {
            if (verbose) message("\nLocating and loading control samples.")
            ids <- .self$controls(treat$distil_id,
                                  verbose = verbose)$distil_id
            control <- .self$toSummarizedExperiment(index = list(seq_len(978),
            ...
            control <- .self$toSummarizedExperiment(
        ...
        stop("rzs expects either 'auto', the pert_desc of the perturbagen or",
             "a Summarized Experiment for the 'control' dataset")
    }
...
Slinky$methods(rzs = function(treat,
    ...
    \\subsection{Parameters}{
    \\itemize{
        \\item{\\code{treat} A SummarizedExperiment containing the 
            treated samples,
            or the pert_desc of desired perturbagen. See details. }
        \\item{\\code{control} An SummarizedExperiment containing the control 
            samples, or the pert_desc of desired controls. Default is 'auto'. 
            See details.}
        \\item{\\code{where_clause} If treat is a pert_desc, further query
            terms may be specified here (e.g. \\code{pert_type=\"trt_sh\"}).}
    ...
    zs <- as.data.frame(SummarizedExperiment::colData(treat)) %>%
        dplyr::group_by(rna_plate) %>%
        dplyr::do(z =
            .self$.zs(assays(treat)[[1]]
                        [, which(treat$rna_plate %in% .$rna_plate)],
                      assays(control)[[1]]
                        [, which(control$rna_plate %in% .$rna_plate)]))
    # flatten structure to matrix
