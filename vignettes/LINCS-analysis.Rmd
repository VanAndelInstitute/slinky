...
For example, consider the high level method function \\code{load}.  If 
...
These steps can be adjusted with additional optional arguments to 
\\code{load}.  For users more familiear with the underlying data resources, 
much more granular control can be achieved through calls to the lower level 
functions including \\code{toSummarizedExperiment}, \\code{clue}, and 
\\code{readGCTX}.  Examples of these and other functions provided by this 
...
data <- sl$readGCTX(index = list(NULL, col.ix))
```
...
amox_gold_data <- sl$readGCTX(index = list(NULL, ix))
```
...
amox_gold_sumex <- sl$toSummarizedExperiment(index = list( NULL, ix))
...
amox_gold_sumex <- sl$toSummarizedExperiment(ids = amox_gold)

```

In fact, you could have skipped the trouble of looking up the ids in the first
place and simply passed the `where_clause` to `toSummarizedExperiment` 
directly.

```{r, echo=TRUE, message=F, warning=T, eval=TRUE}

amox_gold_sumex <- sl$toSummarizedExperiment(
...
Note that by default, `toSummarizedExperiment` only loads the L1000 genes, 
so there is no need to specify the row index.  If you desire all the genes, 
simply specify
`inferred=TRUE` when calling `toSummarizedExperiment`.  If you want some other 
subset of genes, you will need to use the `index` method shown previously, and 
not the ids method. This underscores the fact that `toSummarizedExperiment` 
expects exactly one of the `ids`, `index`, or `where_clause` arguments.  
Specifying more than one of those arguments, or none of them, will throw an 
error.
...
fda_pert <- sl$clueInstances(poscon = "omit",
                where_clause = list("pert_type" = "trt_cp", 
                                    "is_gold" = TRUE,
                                    "pert_iname" = 
...
with the full GCTX file we could then load this data as follows:

```{r, eval=FALSE}
fda_gold_sumex <- sl$toSummarizedExperiment(ids = fda_pert, controls = TRUE)
```

Reading all 32891 instances from the full gctx file from
...
veh <- sl$clueVehicle(amox_gold, verbose=FALSE)
...
amox_and_control <- sl$toSummarizedExperiment(
                          ids = c(amox_gold, sl$metadata$inst_id[ix]))
```
...
amox_and_control <- sl$toSummarizedExperiment(ids = c(amox_gold, 
                                                      controls$distil_id))
```
...
```{r eval=TRUE}
amox_and_control <- sl$toSummarizedExperiment(ids = amox_gold, 
                                              controls = TRUE)
```
...
```{r eval=TRUE}
cd_vector <- sl$diffexp(treat = "amoxicillin", 
...
sumex <- sl$toSummarizedExperiment(index = list(seq_len(978), seq_len(131)))
