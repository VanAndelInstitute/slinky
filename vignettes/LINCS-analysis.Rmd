...
amox_gold_eset <- sl$toEset(index = list( NULL, ix))
...
```{r, eval=FALSE}
fda_gold_eset <- sl$toEset(ids = fda_pert, controls = TRUE)
```
...
wd <- "\\\\nasgw.hpc.vai.org\\projects_secondary\\jovinge\\LINCS\\"
sl <- Slinky$new(readLines(paste0(wd, ".api"))[1],
                 paste0(wd, "GSE92742_Broad_LINCS_Level3_INF_mlr12k_n1319138x12328.gctx"))
sl$loadInfo("GSE92742_Broad_LINCS_inst_info.txt.gz")

ix <- which(sl$metadata$pert_iname %in% c("sirolimus",   
                                          "geldanamycin",
                                          "wortmannin",
                                          "trichostatin-a",
                                          "vorinostat") & 
            sl$metadata$cell_id == "VCAP" &
            sl$metadata$pert_time == 24)
ix <- which(sl$colnames() %in% sl$metadata$inst_id[ix])
eset <- sl$toEset(index = list(seq_len(978), ix))




set.seed(100)
ts <- Rtsne(t(Biobase::exprs(eset)), perplexity = 50)
