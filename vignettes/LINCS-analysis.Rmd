...
not use the demo key for actual analysis. **Note:** the data in this 
...
amox_gold_eset <- sl$toSummarizedExperiment(index = list( NULL, ix))
...
amox_gold_eset <- sl$toSummarizedExperiment(ids = amox_gold)
...
amox_gold_eset <- sl$toSummarizedExperiment(
...
fda_pert <- sl$clue.instances(poscon = "omit",
                where_clause=list("pert_type"="trt_cp", 
                                    "is_gold"=TRUE,
...
```{r, eval=FALSE}
fda_gold_eset <- sl$toSummarizedExperiment(ids = fda_pert, controls = TRUE)
```
...
amox_and_control <- sl$toSummarizedExperiment(ids = c(amox_gold, controls$distil_id))
```
...
```{r eval=TRUE}
amox_and_control <- sl$toSummarizedExperiment(ids = amox_gold, controls=TRUE)
```
...
eset <- sl$toSummarizedExperiment(index = list(seq_len(978), seq_len(131)))

set.seed(100)
ts <- Rtsne(t(SummarizedExperiment::assays(eset)[[1]]), perplexity = 10)
tsne_plot <- data.frame(x = ts$Y[,1], 
                        y = ts$Y[,2], 
                        treatment = eset$pert_iname,
                        plate = eset$rna_plate)
...
can be seen in the resulting figure reproduced below, that is not the case. 
