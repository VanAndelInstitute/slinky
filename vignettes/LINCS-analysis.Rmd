...
fda <- sl$query("rep_drugs", where_clause=list("status_source"=list(like = "FDA Orange"),
...
Finally, note that when multiple values are present for a given field (i.e., the JSON returned by clue.io contained an array of values), these values have been concatenated by the `|` character so that they could be placed in a single column of the dataframe.  Without this concatenation, a well formed `data.frame` could not be reliably returned by `query`.

```{r, eval=FALSE}
fda_pert <- sl$query("sigs", 
                     fields = c("pert_iname", "distil_id"), 
...
```{r, eval=FALSE}
l1000 <- sl$query("genes", fields=c("entrez_id", "gene_symbol"), where_clause=list("l1000_type" = "landmark"))
