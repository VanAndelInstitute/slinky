...
In this vignette we perform some basic analyses using the L1000 data expression
data from the LINCS project.  To complete this vignette you will need access to
...
direction analysis as implemented in the `GeoDE` package.  This functionality
is wrapped in the `de.by.plate` (differential expression by plate) function. 
...
You can provide the `de.by.plate` function two expression sets (treated and 
control) to analyze.  The control expression set should cover all the plates 
represented in the trated dataset.  Alternatively, you can simply provide 
the treated samples as an expression set and `de.by.plate` will automatically 
identify and load the corresponding control samples in the same manner as 
discussed above.

Alternatively, you can simply provide a "where_clause" to identify the treated 
samples of interest.  Then `de.by.plate` will load the corresponding data, 
and identify and load the corresponding same-plate controls.

The function returns a data frame that contains one vector of scores per plate.
You can then summarize these scores as you wish.  One approach is to take the 
rank product of the scores to identify those genes that are most consistently 
